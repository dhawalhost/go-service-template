...
#### Chart Structure
Every service (Go API, React frontend) must have its own Helm chart:
```
charts/
├── go-api/
...
### React Frontend
- Multi-stage Dockerfile: `node:20-alpine` (build) → `nginx:1.25-alpine` (serve)
- Inject env vars at build time via CI: `REACT_APP_API_URL`
- Run `npm ci && npm test -- --watchAll=false --coverage` in CI
- Use `nginx.conf` with gzip, cache headers, and security headers
- Helm chart exposes: `image.tag`, `replicaCount`, `ingress`, `config.apiUrl`
